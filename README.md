...
본 프로젝트는 강사(크리에이터)가 강의를 개설하고 수강생이 신청·결제·취소하는 흐름을 다루는 백엔드 시스템입니다. 정원 관리·결제 기한·대기열·환불 규칙 같은 비즈니스 규칙의 정확한 동작이 핵심입니다.
...
- **결정**: 사용자·강의 조합 기준으로 활성 신청이 존재하는 경우 신청을 거부한다(DB 유니크 제약 또는 사전 검증으로 보장).
...
### 8. 인증/사용자 식별 방식
- **관련 요구사항**: 전체 API
- **해석 및 가정**: 과제 가이드에 "인증/인가는 간략히 처리해도 무방하며, userId를 헤더나 파라미터로 전달하는 방식도 허용"으로 명시되어 있다.
- **결정**: 요청 시 사용자 식별자를 쿼리 또는 본문 파라미터로 전달받아 사용자 컨텍스트를 식별한다. Spring Security 등 별도 인증 인프라는 도입하지 않고, 가벼운 요청 전처리 단계에서 사용자 존재와 역할(`ROLE_USER`/`ROLE_CREATOR`)을 확인한다.
...
- **해석 및 가정**: 정원이 다 찬 시점에 강의 상태를 `CLOSED`로 바꾸면 대기열 진입조차 불가능해져 #10의 취지와 충돌한다. `CLOSED`는 "모집 자체가 종료됨"(종료일 경과 또는 CREATOR 명시적 마감)에만 한정해야 의미가 깔끔하다.
...
- **결정**: **1단계(현재 구현)** — 강의 레코드에 비관적 락을 걸어 신청 카운트 갱신 트랜잭션을 직렬화한다. 보조로 사용자·강의 조합의 활성 상태에 DB 유니크 제약을 두어 중복 신청(### 5)의 race condition도 차단한다. **2단계(향후 마이그레이션)** — 트래픽 확장 시 Redis 기반 분산 락으로 전환을 고려한다. 마이그레이션 이점: ① DB 락 경합 감소로 신청 처리량 향상, ② 트랜잭션 외부 락이므로 DB 커넥션 점유 시간 단축, ③ 다중 인스턴스 환경에서도 자연스럽게 동작(JVM 단위 락의 한계 회피), ④ TTL 기반 락으로 데드락 위험 ↓. 단점: 외부 인프라(Redis) 운영 부담 증가 및 락-DB 정합성 별도 보장 필요.
...
| 재시작·복구 | 강력함 (다시 돌릴 때 이어서 처리) | 단순함 (작업이 다시 돌아도 안전하면 충분) |
...
| 계층 가시성 | 한눈에 보임 | 도메인 안에서 보임 | 의존성 방향까지 명확 |
| 도메인 응집 | 클래스명 접두로만 표현 | 폴더 단위로 묶임 | 폴더 단위로 묶임 |
| 작은 규모 적합성 | 매우 적합 | 적정 (도메인 수가 많을 때 유리) | 부적합 (추상화 비용 과다) |
...
3. Enrollment 테이블 : `(user_id, course_id)` (유니크)
이유 : 동일 사용자가 동일 강의에 활성 상태로 중복 신청하지 못하도록 막는다(`### 5`). 유니크 제약이 동시 신청 race condition까지 차단한다. 앞쪽 컬럼이 `user_id`라 내 수강 신청 목록 조회(#9)에도 그대로 활용된다.
